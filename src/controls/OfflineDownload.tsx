...
export function OfflineDownload({host}:{host:HTMLElement|null}) {
  ...
  return host?createPortal(<div className="offline-download"><button className="original-button" disabled={status==='saving'||status==='ready'} onClick={()=>void download()}>{status==='ready'?'✓ Ready for offline play':status==='saving'?`Downloading… ${progress}%`:status==='error'?'Retry download':'Download offline'}</button>
