...
All twelve courses load, but this is not complete behavioral parity. Levels 2–12 are explicitly marked as mechanics in progress. Their driven swinging mechanisms, linked bridges and other scripted machinery still use imported static meshes where no behavior adapter exists. Fans, pushers and five passive hinge types now have behavior adapters. The [module inventory](original-fans.md#remaining-module-inventory) identifies the remaining gaps. They are not guaranteed completable.
