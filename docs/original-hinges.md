...
Modules 19, 25, 30 and 37 begin frozen and activate within the recovered 50-original-unit horizontal proximity. Merely calling Rapier `sleep()` was insufficient because attaching a joint could wake the body immediately. The runtime now holds the initial pose fixed until proximity activation, then makes it dynamic. Module 41 starts dynamic, as specified in its original script. Sector resets restore the body's initial pose and frozen state while retaining its correct hinge anchor. No artificial motors or animation-driven opening are added.
