...
const distance=Number(process.argv[2]??12),settle=Number(process.argv[3]??60)
if(!Number.isFinite(distance)||distance<=0||!Number.isInteger(settle)||settle<0||settle>=240)throw new Error("Invalid approach distance or settle frames")
const results:unknown[]=[]
for(const level of [2,3,4,6,7,8,9,10,11]){
 const runtime=await setup(level)
 try{for(const parent of runtime.course.objects.filter(o=>o.name.startsWith('P_Modul_29_')))for(const sign of [1,-1]){
  const frame=new THREE.Matrix4().fromArray(parent.matrix),origin=new THREE.Vector3().setFromMatrixPosition(frame),axis=new THREE.Vector3().setFromMatrixColumn(frame,0).multiplyScalar(sign)
  const sector=Number(runtime.course.groups.find(g=>/^Sector_/.test(g.name)&&g.members.includes(parent.id))!.name.slice(-2))
  runtime.reset(sector,'wood',new THREE.Vector3(-distance*sign,3,.21).applyMatrix4(frame).toArray())
  let entrance=false,touched=false,exit=false
  for(let tick=0;tick<240;tick++){
   ...
   if(p.clone().sub(origin).dot(axis)>12&&onAuthoredFloor(runtime))exit=true
  }
  results.push({level,name:parent.name,sign,entrance,touched,exit,position:runtime.player.pose.position})
 }}finally{runtime.dispose()}
}

console.log(JSON.stringify({conditions:{material:"wood",fps:60,settleFrames:settle,driveFrames:240-settle,startLocal:[-distance,3,.21],exitDistance:12},results},null,2))
